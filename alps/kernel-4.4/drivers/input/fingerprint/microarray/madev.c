...
static unsigned int MA_DRV_VERSION = 0x00004006;
...
int init_connect(void){
    ...
    for(i=0; i<4; i++){
		...
		#if defined(MICROARRY_TEE_SUPPORT)
...
int mas_probe(struct spi_device *spi) {
    ...
    if(ret == 1){
       ret = init_connect(); //需要ODM 或者指纹厂商单独实现，功能是读取指纹chipid，并且判断chip id 是否正确
       if(ret == 1){
           MALOGD("read microarray fp id success!");
           set_fp_vendor(FP_VENDOR_MICROARRAY); // FP_VENDOR_YOUSENSOR 具体的值可以参考以上文档
       } else {
           // show error
  	    MALOGD("can't read microarray chip id!");
           goto err5;
       }    
    } else {
             MALOGD("can't enable tee spi");
           // show error
           return -1;
    }
#endif
   ...
   // }
    ...
    MALOGD("end1111");
#if defined(MICROARRY_TEE_SUPPORT)
 fingerprint_set_name("microarray_tee"); //zwl 20180313
#else
 fingerprint_set_name("microarray"); //zwl 20180313
#endif
